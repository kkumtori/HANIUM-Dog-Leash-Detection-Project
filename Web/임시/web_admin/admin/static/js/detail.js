...
function getPageData(pageNumber) {
    // slice 메서드를 사용하여 해당 페이지의 데이터만 추출하기
...
function createTable(pageData) {
    document.getElementById("info_table_tbody").innerHTML = "";
    document.getElementById("info_table_tbody2").innerHTML = "";
    ...
    for(let i=0; i<pageData.length; i++){
        ...
        if (i < 6) {
            ...
            tbody = document.getElementById("info_table_tbody2");
        } else {
            break;
        }
...
function displayNavigation() {
    ...
    if(currentPage != 1) {
        ...
        firstButton.setAttribute('id', 'doubleleftbtn')
        ...
        prevButton.setAttribute('id', 'leftbtn')
    ...
    for(let i=startPage; i<=endPage; i++) {
        ...
        buttonElement.setAttribute('class', 'pagebtn');
        if(i == currentPage){
            buttonElement.classList.add("current-page"); // 현재 페이지 강조
        }
        ...
        paginationDiv.appendChild(buttonElement);
}

   // 다음과 마지막 버튼
    if(currentPage != totalPages) {
        let nextButton=document.createElement("button");
        nextButton.setAttribute('id', 'rightbtn')
        ...
        lastButton.setAttribute('id', 'doublerightbtn')
...
function displayPage(pageNumber) {
    let pageData = getPageData(pageNumber);
